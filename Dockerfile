...
ENV PATH=/working/software/miniconda/bin:$PATH
...
RUN wget --quiet https://repo.anaconda.com/miniconda/Miniconda3-latest-Linux-x86_64.sh -O ~/miniconda.sh && \
    /bin/bash ~/miniconda.sh -b -p /working/software/miniconda && \
    rm ~/miniconda.sh && \
    ln -s /working/software/miniconda/etc/profile.d/conda.sh /etc/profile.d/conda.sh && \
